...
class WeightedGraph {
    ...
    private void makeSet(int v, int[] parent, int[] size) {
        parent[v] = v;
        size[v] = 1;
    }
    ...
    public void solve() {
        int[] parent = new int[this.numVertices + 1];
        int[] size = new int[this.numVertices + 1];
 
        for (int i = 0; i <= this.numVertices; i++) {
            makeSet(i, parent, size);
        }
        int components = this.numVertices;
        int maxSize = 1;
        ...
        for (int i = 0; i < this.numEdges; ++i) {
            ...
            maxSize = Integer.max(size[findSet(u, parent)], maxSize);
 
            if (components < 1)
                components = 1;
            obj.println(components + " " + maxSize);
        }
